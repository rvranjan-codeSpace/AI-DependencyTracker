...
def createJQLs(
    ...
    format_msg=prompt.format_messages(similar_JQL_examples=similar_JQL_examples,user_query=inputs["user_input"],format_instructions=JQLQueryParser.get_format_instructions())
...
def getJQLPrompt():
    ...
    ("system", "You are an expert in creating executable JQL queries for JIRA. Generate a precise JQL that can be used downstream for further processing."),
    ("system", """
        Here are some examples of JQLs for reference:
        {similar_JQL_examples}
    """),
    ("user", """Based on the examples, generate an executable JQL for the following query: {user_query}. 
     Ensure the JQL is in the correct format and can be executed in JIRA.
     {format_instructions}
     Mistaked you have made in pasts:
     1)Your JQL = issue in linkedIssues(XSP-27, "is blocked by") AND issuetype = "User Story"
     Mistake: issueType should be Story
     Correct JQL:issue in linkedIssues(XSP-27, "is blocked by") AND issuetype = "Story"
     """   )
])
    return prompt
